...
class TheBrain:
    ...
    def _usps8(self, n=300, rot=0):
        """GENUINE OOD: USPS handwritten digits -- a DIFFERENT dataset than the
        MNIST the cortex trained on (not a held-out split of the same data).
        Cross-distribution transfer; optionally rotated for extra shift."""
        key = f"_usps{rot}"
        if not hasattr(self, key):
            from datasets import load_dataset
            from PIL import Image
            ds = load_dataset("flwrlabs/usps", split="train", streaming=True)
            xs, ys = [], []
            for ex in ds:
                ...
                if len(xs) >= n:
                    break
            setattr(self, key, (np.stack(xs), np.array(ys)))
    ...
    def ood_eval(self):
        ...
        for _ in range(na):
            ...
            ok += (self.mul(d, n) == d * n)
        mul_big = 100 * ok / na
        # vision: GENUINE cross-dataset OOD -- USPS digits (not an MNIST split)
        try:
            Xt, Yt = self._usps8(300, rot=0)
            vt = 100 * np.mean([self.cortex.predict(Xt[j]).argmax() == Yt[j] for j in range(len(Yt))])
            Xr, Yr = self._usps8(300, rot=20)
            vr = 100 * np.mean([self.cortex.predict(Xr[j]).argmax() == Yr[j] for j in range(len(Yr))])
        except Exception:
            vt = vr = -1
        return dict(add_big=add_big, mul_big=mul_big, vis_usps=vt, vis_usps_rot=vr)
    ...
    def lifelong(self, chunk=40000, test_n=15000):
        ...
        while not os.path.exists("outputs/STOP"):
            ...
            line = (f"cycle {cyc}: txt {self.read_pos//1000}k  img {self.images_seen}  "
                    ...
                    f"usps {ood['vis_usps']:.0f} usps_rot {ood['vis_usps_rot']:.0f}]")
