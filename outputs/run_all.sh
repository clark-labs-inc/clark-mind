#!/bin/zsh
# Full ARC-AGI-3 public-set benchmark: 25 games x 2 passes, brains keyed by
# GAME-ID prefix (the API shuffles env order per call, so indices are unstable).
...
for pass in 1 2; do
  for g in $GAMES; do
    ...
    .venv-arc/bin/python clark_arc_agent.py --steps 8000 --game $g \
      --brain outputs/brain_v2_$g.pkl --verbose_every 100000 \
      || echo "##### GAME $g FAILED #####"
  done
done
echo "##### ALL DONE #####"
